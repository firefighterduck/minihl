use crate::{
    types::{MonoTypeResult, TypeUnifier, TypingResult, UNIT},
    BinOp, Expr, Literal, MonoType, Type, TypeContext, TypeError, UnOp, Unifiable, Val, BOOL_T,
    FUN, INT_T, LOCATION, PROD, SUM, UNIT_T,
};
...
pub fn type_check_val(val: &Val, ctxt: &mut TypeContext) -> TypingResult {
    match val {
...
pub fn type_check(expr: &Expr, ctxt: &mut TypeContext) -> TypingResult {
    match expr {
        ...
        Expr::InjL(left) => {
            let (left_type, unifiers) = type_check(left, ctxt)?;
            MonoType::constr(SUM, vec![left_type, MonoType::Free(ctxt.new_var())])
                .map(|typ| (typ, unifiers))
        }
        Expr::InjR(right) => {
            let (right_type, unifiers) = type_check(right, ctxt)?;
            MonoType::constr(SUM, vec![MonoType::Free(ctxt.new_var()), right_type])
                .map(|typ| (typ, unifiers))
        }
        Expr::Case {
            ...
            type_check_sum(match_expr, ctxt, |l_type, r_type, ctxt, mut unifiers| {
                let return_type = MonoType::Free(ctxt.new_var());
                let expected_case_left =
                    MonoType::constr(FUN, vec![l_type.clone(), return_type.clone()])?;
                let expected_case_right = MonoType::constr(FUN, vec![r_type.clone(), return_type])?;

                // Left case.
                let (mut l_case_type, mut unifiers1) = type_check(left_case, ctxt)?;
                unifiers.append(&mut unifiers1);
                let unifier1 = l_case_type.unify_with(&expected_case_left)?;
                l_case_type.instantiate_from_unifier(&unifier1);
                unifiers.push(unifier1);

                // Right case.
                let (mut r_case_type, mut unifiers2) = type_check(right_case, ctxt)?;
                unifiers.append(&mut unifiers2);
                let unifier2 = r_case_type.unify_with(&expected_case_right)?;
                r_case_type.instantiate_from_unifier(&unifier2);
                unifiers.push(unifier2);

                // Make sure, both cases produce the same return type.
                let mut ret_type_left = l_case_type.args(&FUN)?.get(2).unwrap().clone();
                let ret_type_right = r_case_type.args(&FUN)?.get(2).unwrap();
                let unifier = ret_type_left.unify_with(ret_type_right)?;
                ret_type_left.instantiate_from_unifier(&unifier);
                ctxt.fix_from_unifier(&unifier);
                unifiers.push(unifier);

                Ok((ret_type_left, unifiers))
            })
        }
        Expr::Fork(thread) => {
        ...
        Expr::CmpXchg {
            ...
            let expected_loc_type = MonoType::constr(LOCATION, vec![expected_val_type.clone()])?;
            let unifier2 = loc_type.unify_with(&expected_loc_type)?;
            expected_val_type.instantiate_from_unifier(&unifier2);
            ctxt.fix_from_unifier(&unifier2);
            unifiers1.push(unifier2);

            let ret_type = MonoType::constr(PROD, vec![expected_val_type, BOOL_T])?;
            Ok((ret_type, unifiers1))
        }
        Expr::Xchg {
            location,
            new_expr_val,
